...
class MazeGUI:
            ...
            for i in range(0, self.dim - 1):
                for j in range(0, self.dim - 1):
                    ...
                    if fire_maze[i][j] != 1 and fire_array[i][j] != 2:
                        ...
                        self.fire_maze[i][j] = FireNode(1, 0.0) # represents an obstacle
                    else:
                        if fire_array_copy[i+1][j] == 2:
                            fire += 1
                        if fire_array_copy[i-1][j] == 2 and i != 0:
                        ...
                        if fire_array_copy[i][j-1] == 2 and j != 0:
                            fire += 1
                        prob = 1 - ((1 - q)**fire)
                        self.fire_maze[i][j] = FireNode(0, prob) # represents the free cell with no obstacle or fire
    ...
    def fire_route_search(self, start): # implemented using a modified bfs
        ...
        # also defined is a start for everytime you want to start the maze at a certain point ((0, 0) to start then all according to bfs)
        goal = (len(arr) - 1, len(arr) - 1) # ending will stay consistent 
        ...
        if self.fire_maze[start[0]][start[1]].value == 2:
            print("Start of maze on fire, can't proceed")
            return []
        elif self.fire_maze[goal[0]][goal[1]].value == 2:
            print("Ending on fire can't proceed")
            return []

        # now iterate through the fringe to check for the path
        while len(fringe) > 0:
            
            if self.fire_maze[goal[0]][goal[1]].value == 2:
                print("Ending on fire can't proceed")
                return []

            current = fringe.popleft()

            if self.fire_maze[current[0]][current[1]] == 2:
                print("OUCH IM ON FIRE")
                return []
            ...
            if current == goal:
                ...
                if self.check_valid_bounds(-1, 0, current, arr) and self.fire_maze[current[0] - 1][current[1]].value == 0 and  self.fire_maze[current[0] - 1][current[1]].prob_fire < 0.3 and visited[current[0] - 1][current[1]] == False and (current[0] - 1, current[1]) not in fringe:
                ...
                if self.check_valid_bounds(0, -1, current, arr) and self.fire_maze[current[0]][current[1] - 1].value == 0 and self.fire_maze[current[0]][current[1] - 1].prob_fire < 0.3 and visited[current[0]][current[1] - 1] == False and (current[0], current[1] - 1) not in fringe:
