...
class Maze:
        ...
        while len(self.fringe) > 0:
            ...
            if current == (goal[0], goal[1]):
                ...
                if current not in self.visited:
                    if current[1] > 0:
                        if self.tracking_array[current[0]][current[1] - 1] == 0 and (
                        current[0], current[1] - 1) not in self.fringe and (
                        current[0], current[1] - 1) not in self.visited:
                            self.fringe.append((current[0], current[1] - 1))
                        if current[0] != self.dim - 1 and self.tracking_array[current[0] + 1][current[1]] == 0 and (
                        current[0] + 1, current[1]) not in self.fringe and (
                        current[0] + 1, current[1]) not in self.visited:
                            self.fringe.append((current[0] + 1, current[1]))
                        if current[1] != self.dim - 1 and self.tracking_array[current[0]][current[1] + 1] == 0 and (
                        current[0], current[1] + 1) not in self.fringe and (
                        current[0], current[1] + 1) not in self.visited:
                            self.fringe.append((current[0], current[1] + 1))
                        if current[0] == self.dim - 1 and self.tracking_array[current[0] - 1][current[1]] == 0 and (
        ...
        while len(fringe) > 0:
            ...
            if current == goal:
                ...
                if self.check_valid_bounds(0, -1, current, arr) and arr[current[0]][current[1] - 1] == 0 and \
                        self.fire_maze[current[0]][current[1] - 1].fire_prob < .3 and visited[current[0]][
                    current[1] - 1] == False and (current[0], current[1] - 1) not in fringe:
    ...
    def strategy1(self, flame):

        self.generate_fire_maze(flame)  # ignite the first fire in the maze
        ...
        for i in range(0, x):  # the loop is run for every step of the path
            ...
            if curr[0] == dimension and curr[1] == dimension:  # if current is at the goal cell then we return true and agent has reached the goal
                return True
            self.generate_fire_maze(flame)  # update the fire in maze
...
def running_tests():
    ...
    tests = [(100, 0.3, 0.1)]  # (, (100, 0.1, 0.2) (100, 0.3, 0.3), (100, 0.3, 0.4), (100, 0.3, 0.5), (100, 0.3, 0.6)
