...
def dfs(start, goal,fringe,child1,child2,child3,child4,visited):
    first=start.split(",")
    second=goal.split(",")
        ...
        if current == target:
            ...
            if current not in visited:
                if current[1] == 0:
                    ...
                    if (current[0] == 0 and current[1] != 0) or (current[0]==rows-1 and current[1]!=0):
                        child1.append((current[0], current[1]-1))
                        if current[0] == 0:
                            child2.append((current[0]+1, current[1]))
                        elif current[0] == rows-1:
                            child2.append((current[0]-1, current[1]))
                        if current[1] != cols-1:
    ...
    return False
print(dfs(start,goal,fringe,child1,child2,child3,child4,visited))
