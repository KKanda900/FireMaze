        ...
        if current == target:
            ...
            if current not in visited and maze[current[0]][current[1]] == 0:
                ...
                if (
                    current[1] == 0
                    and maze[a][b + 1] != 1
                    and ((a and b + 1) in range(0, dim))
                ):
                    child1.append((current[0], current[1] + 1))
                    if (
                        current[0] != dim - 1
                        and maze[a + 1][b] != 1
                        and ((a + 1 and b) in range(0, dim))
                    ):
                    ...
                    if (current[0] == 0 and current[1] != 0) or (
                        current[0] == dim - 1 and current[1] != 0
                    ):
                        if maze[a][b - 1] != 1:
                        ...
                        if (
                            len(child3) > 0
                            and child3[0] not in fringe
                            and child3[0] not in visited
                        ):
                        ...
                        if (
                            len(child4) > 0
                            and child4[0] not in fringe
                            and child4[0] not in visited
                        ):
                            fringe.append((child4[len(child4) - 1]))
                if (
                    len(child1) > 0
                    and child1[0] not in fringe
                    and child1[0] not in visited
                ):
                    fringe.append((child1[len(child1) - 1]))

                if (
                    len(child2) > 0
                    and child2[0] not in fringe
                    and child2[0] not in visited
                ):
...
def start():
    ...
    answer = input(
        "Do you want to run DFS on this generated maze for any two points: type yes or no"
    )
    if answer.lower() == "yes":
